...
////Calls upon the "EditUserInformationForm" function when the "Aanpassen" knop is pressed.
if (isset($_POST["DeleteBtn"]))
{
  if ($_POST["DeleteBtn"] && $_POST["id"])
  {
    if ($_POST["DeleteBtn"] = "Delete")
    {
      if (session_status() == PHP_SESSION_NONE) 
      {
        //als er geen sessie was start hij er een. 
        session_start();
      }
        Connect(); 
      ...
      if ($stmt->execute())
      {
        UpdatePriority($BrideLoginCode); 
        header("Location: bruidspaar.php"); 
      }
    }
  }
} 
...
function EditUserInformation($LicenseName, $Description, $InstallDesc, $ExpirationDate, $UserID)
{
  date_default_timezone_set('Europe/Amsterdam');
  $CurrentDate = date('Y/m/d');
