  ...
  echo "
    <div class=\"col-7\">
      <form action=\"UserAdministration.php\" method=\"post\">
        ...
        <input type=\"radio\" name=\"NewRightsRead\" value=\"0\" checked>
        <label for=\"Lezer\">Lezer</label><br>
        <input type=\"radio\" name=\"NewRightsAdm\" value=\"1\">
...
function EditUserInformation($UserID)
{
  ...
  $Password2 = validate($Password2);
  $Hashed = password_hash($Password, PASSWORD_DEFAULT);
  if (isset($_POST['NewRightRead']))
  {
    $Rights = $_POST["NewRightsRead"];
  }
  if (isset($_POST['NewRightAdm']))
  {
    $Rights = $_POST["NewRightsAdm"];
  }
  ...
  try
  {
    ...
    $PastUsername = "SELECT UniekeLoginNaam FROM gebruiker WHERE GebruikerID = $UserID";
    ...
    {
      if (empty($Username) || $Username == $PastUsername)
      {
        $Username = $PastUsername; 
      }
      if (empty($Password))
      {
        $PastPassword = "SELECT Wachtwoord FROM gebruiker WHERE GebruikerID = $UserID";
        $Password = $PastPassword; 
      }
      if (empty($Password2))
      {
        $PastPassword2 = "SELECT Wachtwoord FROM gebruiker WHERE GebruikerID = $UserID";
        $Password2 = $PastPassword2; 
      }
      if (empty($RightsRead) && empty($RightsAdm))
      {
        $PastRights = "SELECT Rechten FROM gebruiker WHERE GebruikerID = $UserID";
        $Rights = $PastRights; 
      }
