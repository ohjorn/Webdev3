...
function EditUserInformation($NewUsername, $NewPassword, $NewPassword2, $NewRights, $UserID)
{
  ...
  try
  {
    ...
    if ((!($Result == $Username)) && ($Password == $Password2) && (strlen($Username) > 2))
    {
      if (empty($Username))
      {
        $PastUsername = "SELECT UniekeLoginNaam FROM gebruiker WHERE Un";
        $Username = 
      }
      $Checked = $_POST["id"];
      $sql = "UPDATE `gebruiker` SET UniekeLoginNaam=(`UniekeLoginNaam`, `Wachtwoord`, `Rechten`) VALUES (NULL, :Username, :Password, :Rights);";
